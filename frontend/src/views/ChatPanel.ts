...
You can also select code in your editor and ask me about it directly!`,
    ...
    for (const pattern of filePatterns) {
      ...
      if (matches) {
        ...
        const filteredMatches = matches.filter(match => {
          ...
          if (commonFalsePositives.some(falsePositive =>
            cleanMatch.toLowerCase().includes(falsePositive.toLowerCase())
          )) {
            return false;
          }
  ...
  private isLikelyFileReference(text: string): boolean {
...
<body>
  ...
  private static readonly JAVASCRIPT_ESCAPE_FUNCTION = String.raw`
